...
class FixaProxy {
    ...
    static func obterTodos(_ callback: @escaping ([FixaResponse]) -> ()) {
        ...
        Alamofire.request(
            "https://despesas-despesas.a3c1.starter-us-west-1.openshiftapps.com/api/tipo/fixas?ano=0&mes=0",
            method: .get,
            headers: headers).responseSwiftyJSON{ response in
                var resultado = [FixaResponse]()
                
                response.result.value?.forEach { _, json in
                    var Fixa = FixaResponse()
                    
                    Fixa.id = UUID(uuidString: json["id"].string!)
                    Fixa.nome = json["nome"].string!
